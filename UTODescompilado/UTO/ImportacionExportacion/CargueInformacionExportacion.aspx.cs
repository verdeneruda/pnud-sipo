...
// Type: UTO.ImportacionExportacion.CargueInformacionExportacion
...
{
  public class CargueInformacionExportacion : BaseModulo
  {
    protected ToolkitScriptManager ToolkitScriptManager1;
    protected Label labelCargueExportacion;
    ...
    {
      get => (DataTable) this.Session["CargueInformacionExportacion.Consulta"];
      set => this.Session["CargueInformacionExportacion.Consulta"] = (object) value;
    }
    ...
    protected void buttonprocesar_Click(object sender, EventArgs e)
    {
      ...
      try
      {
        ...
        CConsultaImportacion.EliminaDatosTemporalExportacion(this.MyDataContext.Connection.ConnectionString);
        List<string[]> csv = this.parseCSV(appSetting + this.FileUpload1.FileName);
        for (int index = 1; index <= csv.Count - 1; ++index)
          this.MyDataContext.tde_TemporalDeclaracionExportacions.InsertOnSubmit(new tde_TemporalDeclaracionExportacion()
          {
        ...
        CConsultaImportacion.CargueArchivoExportacion(int.Parse(this.textAno.Text), this.MyDataContext.Connection.ConnectionString);
        this.TablaConsulta = CConsultaImportacion.BuscarDatosExportacion(this.MyDataContext.Connection.ConnectionString);
    ...
    protected void gridResultado_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
      ...
      this.gridResultado.DataSource = (object) CConsultaImportacion.BuscarDatosExportacion(this.MyDataContext.Connection.ConnectionString);
