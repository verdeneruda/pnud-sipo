...
{
  public class ConceptoVB : BaseModulo
  {
    ...
    protected void LinkNuevo_Click(object sender, EventArgs e)
    {
      try
      {
        ...
        sqlCommand.CommandText = "select max(cvb_Id) from [Iye].[cvb_ConceptoVistoBueno]";
        sqlCommand.Connection.Open();
        this.SqlDataCatalogo.InsertParameters["cvb_Id"].DefaultValue = (this.gridCatalogo.Rows.Count + 1).ToString();
        this.SqlDataCatalogo.InsertParameters["cvb_Descripcion"].DefaultValue = "Nueva descripcion";
    ...
    protected void gridCatalogo_RowCommand(object sender, GridViewCommandEventArgs e)
    {
      ...
      try
      {
        ...
        this.SqlDataCatalogo.DeleteParameters["cvb_Id"].DefaultValue = int.Parse(this.gridCatalogo.DataKeys[int.Parse((string) e.CommandArgument) - pageIndex * pageSize].Value.ToString()).ToString();
