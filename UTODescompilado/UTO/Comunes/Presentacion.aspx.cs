...
{
  public class Presentacion : BaseModulo
  {
    ...
    protected void LinkNuevo_Click(object sender, EventArgs e)
    {
      try
      {
        ...
        sqlCommand.CommandText = "select max(psu_Id) from [Uto].[psu_PresentacionSustancia]";
        sqlCommand.Connection.Open();
        this.SqlDataCatalogo.InsertParameters["psu_Id"].DefaultValue = ((int) sqlCommand.ExecuteScalar() + 1).ToString();
        this.SqlDataCatalogo.InsertParameters["psu_Descripcion"].DefaultValue = "Nueva descripcion";
    ...
    protected void gridCatalogo_RowCommand(object sender, GridViewCommandEventArgs e)
    {
      ...
      try
      {
        ...
        this.SqlDataCatalogo.DeleteParameters["psu_Id"].DefaultValue = int.Parse(this.gridCatalogo.DataKeys[int.Parse((string) e.CommandArgument) - pageIndex * pageSize].Value.ToString()).ToString();
