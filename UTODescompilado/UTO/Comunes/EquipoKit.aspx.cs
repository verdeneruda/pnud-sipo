...
{
  public class EquipoKit : BaseModulo
  {
    ...
    protected void LinkNuevo_Click(object sender, EventArgs e)
    {
      try
      {
        ...
        sqlCommand.CommandText = "select max(eki_Id) from [Rrr].[eki_EquipoKit]";
        sqlCommand.Connection.Open();
        this.SqlDataCatalogo.InsertParameters["eki_Id"].DefaultValue = ((int) sqlCommand.ExecuteScalar() + 1).ToString();
        this.SqlDataCatalogo.InsertParameters["eki_Descripcion"].DefaultValue = "Nueva descripcion";
        this.SqlDataCatalogo.InsertParameters["kco_Id"].DefaultValue = "1";
        this.SqlDataCatalogo.InsertParameters["eki_Costo"].DefaultValue = "0";
        this.SqlDataCatalogo.InsertParameters["eki_Disponible"].DefaultValue = "0";
    ...
    protected void gridCatalogo_RowCommand(object sender, GridViewCommandEventArgs e)
    {
      ...
      try
      {
        ...
        this.SqlDataCatalogo.DeleteParameters["eki_Id"].DefaultValue = int.Parse(this.gridCatalogo.DataKeys[int.Parse((string) e.CommandArgument) - pageIndex * pageSize].Value.ToString()).ToString();
