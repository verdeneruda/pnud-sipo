  ...
  public class Pais : BaseModulo
  {
    ...
    protected void LinkNuevo_Click(object sender, EventArgs e)
    {
      try
      {
        ...
        sqlCommand.CommandText = "select max(pas_Id) from [Uto].[pas_Pais]";
        sqlCommand.Connection.Open();
        this.SqlDataCatalogo.InsertParameters["pas_Id"].DefaultValue = ((int) sqlCommand.ExecuteScalar() + 1).ToString();
        this.SqlDataCatalogo.InsertParameters["pas_Descripcion"].DefaultValue = "Nueva descripcion";
    ...
    protected void gridCatalogo_RowCommand(object sender, GridViewCommandEventArgs e)
    {
      ...
      try
      {
        ...
        this.SqlDataCatalogo.DeleteParameters["pas_Id"].DefaultValue = int.Parse(this.gridCatalogo.DataKeys[int.Parse((string) e.CommandArgument) - pageIndex * pageSize].Value.ToString()).ToString();
