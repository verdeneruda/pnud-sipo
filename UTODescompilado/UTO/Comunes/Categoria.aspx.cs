...
{
  public class Categoria : BaseModulo
  {
    protected ToolkitScriptManager ToolkitScriptManager1;
    protected Label labelCatalogo;
    ...
    protected void LinkNuevo_Click(object sender, EventArgs e)
    {
      try
      {
        ...
        sqlCommand.CommandText = "select max(cat_Id) from [Cer].[cat_Categoria]";
        sqlCommand.Connection.Open();
        this.SqlDataCatalogo.InsertParameters["cat_Id"].DefaultValue = ((int) sqlCommand.ExecuteScalar() + 1).ToString();
        this.SqlDataCatalogo.InsertParameters["cat_Descripcion"].DefaultValue = "Nueva descripcion";
        this.SqlDataCatalogo.InsertParameters["cat_Tipo"].DefaultValue = "T";
    ...
    protected void gridCatalogo_RowCommand(object sender, GridViewCommandEventArgs e)
    {
      ...
      try
      {
        ...
        this.SqlDataCatalogo.DeleteParameters["cat_Id"].DefaultValue = int.Parse(this.gridCatalogo.DataKeys[int.Parse((string) e.CommandArgument) - pageIndex * pageSize].Value.ToString()).ToString();
