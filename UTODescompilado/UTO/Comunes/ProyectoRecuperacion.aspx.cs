...
// Type: UTO.Comunes.ProyectoRecuperacion
...
{
  public class ProyectoRecuperacion : BaseModulo
  {
    ...
    protected void LinkNuevo_Click(object sender, EventArgs e)
    {
      try
      {
        ...
        sqlCommand.CommandText = "select max(pre_Id) from [Rrr].[pre_ProyectoRecuperacion]";
        sqlCommand.Connection.Open();
        this.SqlDataCatalogo.InsertParameters["pre_Id"].DefaultValue = ((int) sqlCommand.ExecuteScalar() + 1).ToString();
        this.SqlDataCatalogo.InsertParameters["pre_Descripcion"].DefaultValue = "Nueva descripcion";
    ...
    protected void gridCatalogo_RowCommand(object sender, GridViewCommandEventArgs e)
    {
      ...
      try
      {
        ...
        this.SqlDataCatalogo.DeleteParameters["pre_Id"].DefaultValue = int.Parse(this.gridCatalogo.DataKeys[int.Parse((string) e.CommandArgument) - pageIndex * pageSize].Value.ToString()).ToString();
