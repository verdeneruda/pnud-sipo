...
// Type: UTO.Comunes.DocumentoVerificar
...
{
  public class DocumentoVerificar : BaseModulo
  {
    ...
    protected void LinkNuevo_Click(object sender, EventArgs e)
    {
      try
      {
        ...
        sqlCommand.CommandText = "select max(dve_Id) from [Rrr].[dve_DocumentoVerificar]";
        sqlCommand.Connection.Open();
        this.SqlDataCatalogo.InsertParameters["dve_Id"].DefaultValue = ((int) sqlCommand.ExecuteScalar() + 1).ToString();
        this.SqlDataCatalogo.InsertParameters["dve_Descripcion"].DefaultValue = "Nueva descripcion";
        this.SqlDataCatalogo.InsertParameters["dve_Estado"].DefaultValue = "3";
    ...
    protected void gridCatalogo_RowCommand(object sender, GridViewCommandEventArgs e)
    {
      ...
      try
      {
        ...
        this.SqlDataCatalogo.DeleteParameters["dve_Id"].DefaultValue = int.Parse(this.gridCatalogo.DataKeys[int.Parse((string) e.CommandArgument) - pageIndex * pageSize].Value.ToString()).ToString();
